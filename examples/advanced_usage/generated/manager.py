...
class ConfigManager:
    ...
    @property
    def app(self) -> AppSchema:
        """Get app configuration"""
        if not hasattr(self, '_app_cache'):
            self._app_cache = AppSchema(self._data.get('app', {}))
        return self._app_cache

    @property
    def services(self) -> ServicesSchema:
        """Get services configuration"""
        if not hasattr(self, '_services_cache'):
            self._services_cache = ServicesSchema(self._data.get('services', {}))
        return self._services_cache

    @property
    def monitoring(self) -> MonitoringSchema:
        """Get monitoring configuration"""
        if not hasattr(self, '_monitoring_cache'):
            self._monitoring_cache = MonitoringSchema(self._data.get('monitoring', {}))
        return self._monitoring_cache

    @property
    def deployment(self) -> DeploymentSchema:
        """Get deployment configuration"""
        if not hasattr(self, '_deployment_cache'):
            self._deployment_cache = DeploymentSchema(self._data.get('deployment', {}))
