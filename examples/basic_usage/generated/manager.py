...
class ConfigManager:
    ...
    @property
    def server(self) -> ServerSchema:
        """Get server configuration"""
        if not hasattr(self, '_server_cache'):
            self._server_cache = ServerSchema(self._data.get('server', {}))
        return self._server_cache

    @property
    def database(self) -> DatabaseSchema:
        """Get database configuration"""
        if not hasattr(self, '_database_cache'):
            self._database_cache = DatabaseSchema(self._data.get('database', {}))
        return self._database_cache

    @property
    def logging(self) -> LoggingSchema:
        """Get logging configuration"""
        if not hasattr(self, '_logging_cache'):
            self._logging_cache = LoggingSchema(self._data.get('logging', {}))
        return self._logging_cache
